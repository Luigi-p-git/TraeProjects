...
export default function AddSubscriptionModal({ isOpen, onClose, onAdd }: AddSubscriptionModalProps) {
  ...
  // Function to automatically assign logo based on service name
  const getLogoForService = (serviceName: string): string => {
    const name = serviceName.toLowerCase()
    if (name.includes('spotify')) return '/logos/spotify.svg'
    if (name.includes('netflix')) return '/logos/netflix.svg'
    if (name.includes('youtube')) return '/logos/youtube.svg'
    if (name.includes('adobe')) return '/logos/adobe.svg'
    if (name.includes('github')) return '/logos/github.svg'
    if (name.includes('figma')) return '/logos/figma.svg'
    if (name.includes('apple') || name.includes('icloud')) return '/logos/apple.svg'
    if (name.includes('discord')) return '/logos/discord.svg'
    if (name.includes('slack')) return '/logos/slack.svg'
    if (name.includes('amazon') || name.includes('prime')) return '/logos/amazon-prime.svg'
    if (name.includes('microsoft') || name.includes('office') || name.includes('365')) return '/logos/microsoft.svg'
    if (name.includes('dropbox')) return '/logos/dropbox.svg'
    if (name.includes('hulu')) return '/logos/hulu.svg'
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const autoLogo = getLogoForService(formData.name)
                    ...
                    <div className="mb-8">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {popularServices.map((service) => (
                          ...
                          >
                            <div className="flex items-center space-x-3">
                              <div className="w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center group-hover:bg-white/20 transition-colors overflow-hidden">
                                {service.logo ? (
                                  <img 
                                    src={service.logo} 
                                    alt={service.name} 
